		...
		$scripts = array(
			array(
				...
				'strings' => array(
					...
					'successCopyText' => esc_html__('# field(s) successfully copied to', 'gf_bulk_actions_pro'),
					'successCopyConfirmText' => esc_html__('View form', 'gf_bulk_actions_pro'),
					'successCopyCancelText' => esc_html__('Continue editing', 'gf_bulk_actions_pro'),
			...
			<div id="form_fields_wrap">
				<script type="text/template" id="fields_copy_template">
					<form id="gf_bulk_actions_fields_copy" method="post" action="<?php echo self_admin_url('admin.php?page='.$this->_slug); ?>" novalidate="novalidate">
						...
						<select name="bulk_fields_copy_target_form" id="bulk_fields_copy_target_form">
							<?php
							foreach($forms as $form):
				...
				<form id="gf_bulk_actions_fields_edit" method="post" action="<?php echo self_admin_url('admin.php?page='.$this->_slug); ?>" novalidate="novalidate">
					...
					<table id="bulk_fields_edit" class="form-table">
						<thead>
						<tr valign="top">
							<th style="width:55%;"><?php _e('Field Label', 'gf_bulk_actions_pro'); ?></th>
							<th><?php _e('CSS Class', 'gf_bulk_actions_pro'); ?></th>
				...
				<ul id="form_fields" class="fields">
					...
					foreach($fields as $field):
						...
						?>
						<li class="postbox field <?php echo $field_class_extra; ?>" id="field_<?php echo $field['id']; ?>" data-custom-cls="<?php echo $field_custom_cls; ?>" data-id="<?php echo $field['id']; ?>" data-type="<?php echo $field['type']; ?>">
...
<?php
	...
	public function handle_bulk_actions_submit() {
		...
		if (isset($_POST['bulk_edit_action']) && $_POST['bulk_edit_action'] == 'update') {
			...
			$new_update_fields = (!empty($form_id) && isset($_POST['updated_fields']) && !empty($_POST['updated_fields'])) ? json_decode(stripslashes($_POST['updated_fields'])) : NULL;
	...
	public function ajax_copy_fields() {
		...
		if (!isset($_POST['bulk_fields_copy_fields']) || empty($_POST['bulk_fields_copy_fields'])
				...
				|| !isset($_POST['bulk_fields_copy_target_form']) || empty($_POST['bulk_fields_copy_target_form']))
		{
			die();
		}


		$target_form_id = intval($_POST['bulk_fields_copy_target_form']);
		...
		$new_copy_fields = (!empty($source_form_id) && isset($_POST['bulk_fields_copy_fields']) && !empty($_POST['bulk_fields_copy_fields'])) ? json_decode(stripslashes($_POST['bulk_fields_copy_fields'])) : NULL;
		...
		// update conditional logic on incoming forms
        $f = 0;
		foreach($new_copy_fields as $field) {
		...
		echo (is_wp_error($update_result)) ? 'failed' : 'updated';
		exit;
